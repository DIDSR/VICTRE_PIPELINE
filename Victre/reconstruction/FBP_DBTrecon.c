...
int main(int argc, char * argv[]) {
   ...
   double *proji, *temp, *temp2, *proj, *betas_rad,*b_rad, *img, *img_temp,  *img_tr, *img_cmp = NULL;
   float *projdbt, *projdbt_un,*flatfield ;
   double *angles;
   FILE * fid = NULL;
   FILE * fp  = NULL;
   ...
   char b1[MAXSTRLEN], b2[MAXSTRLEN], b3[MAXSTRLEN], b4[MAXSTRLEN], b5[MAXSTRLEN], b6[MAXSTRLEN], b7[MAXSTRLEN], b8[MAXSTRLEN], b9[MAXSTRLEN], b10[MAXSTRLEN], b11[MAXSTRLEN], b12[MAXSTRLEN], b13[MAXSTRLEN] , b14[MAXSTRLEN] ,b15[MAXSTRLEN],b16[MAXSTRLEN], b17[MAXSTRLEN];
  ...
  fp = fopen (argv[1],"r");                                            // accepts parameters file from command line
/////////////////////////////////////////////// Reading Geometry Data //////////////////////////////////////////

//  fscanf (fp, "%i\n%i\n%i\n%lf\n%lf\n%lf\n%lf\n%lf\n%i\n%i\n%i\n%lf\n%lf\n%lf\n%lf\n%lf\n%lf\n", &na, &ns_old, &nt_old, &ds, &dsd, &dso, &offset_sold, &orbit, &phantom_dimx, &phantom_dimy, &phantom_dimz, &phantom_voxsz, &dx, &dz, &offset_xyz[0], &orb_start, &del_orb);
fscanf (fp, "%s\t%i\n%s\t%i\n%s\t%i\n%s\t%lf\n%s\t%lf\n%s\t%lf\n%s\t%lf\n%s\t%lf\n%s\t%i\n%s\t%i\n%s\t%i\n%s\t%lf\n%s\t%lf\n%s\t%lf\n%s\t%lf\n%s\t%lf\n%s\t%lf\n", &b1, &na, &b2, &ns_old, &b3, &nt_old, &b4, &ds, &b5, &dsd, &b6, &dso, &b7, &offset_sold, &b8, &orbit, &b9, &phantom_dimx, &b10, &phantom_dimy, &b11, &phantom_dimz, &b12, &phantom_voxsz, &b13, &dx, &b14,&dz, &b15, &offset_xyz[0], &b16,&orb_start, &b17,&del_orb);
...
fscanf (fp, "%s\n", ffFile);
printf ("Flat field file: %s \n",ffFile);

///////////////PHANTOM DATA///////////////////////////////

  fscanf (fp, "%i\n", &f);

    for (phntm = 0; phntm < f; phntm++)      // looping over all datasets in the list
{   
    fscanf (fp, "%s\t%s\n", inFile, outFile);
   //sprintf(inFile, "./data/phantom/VICTRE/dense/DBT_dense_pcl_%s_25proj.raw", argv[2]);      //set path for the dense input projections
   //sprintf(outFile, "./data/VICTRE_C_output/dense/DBT_dense_pcl_%s_38recon4d.raw", argv[2]); //set path for the dense output volumes
...
/////////////////////////////////////////////////////////////////
   ...
   if (runlvl <= PREWEIGHT) {
   projdbt = (float*)malloc(sizeof(float)*ns_old*nt_old*na);
   ...
   fread(projdbt_un, sizeof(float), ns_old*nt_old*na*down*down, fid);
...
//////////////////////////STARTING STANDARD FBP RECONSTRUCTION //////////////////////////////////////////
 ...
 // Write output image to file
   ...
   free(proj);
   free(betas_rad);
   free(img);free(img_temp) ;   free(img_tr);
