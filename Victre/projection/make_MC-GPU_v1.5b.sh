...
nvcc MC-GPU_v1.5b.cu -o MC-GPU_v1.5b.x -m64 -O3 -use_fast_math -DUSING_MPI -I. -I$CUDA_INC -I $MPI_INCLUDE -L $MPI_LIB -lmpi -lz --ptxas-options=-v -gencode=arch=compute_50,code=sm_50 -gencode=arch=compute_61,code=sm_61 -gencode=arch=compute_70,code=sm_70
## echo building non-MPI version does not work
## nvcc MC-GPU_v1.5b.cu -o MC-GPU_v1.5b.x -m64 -O3 -use_fast_math -I. -I/projects01/sysadmin/stuartb/cuda/cuda-10.2/cuda-samples/common/inc/ -lz --ptxas-options=-v -gencode=arch=compute_50,code=sm_50 -gencode=arch=compute_61,code=sm_61
## nvcc MC-GPU_v1.5b.cu -o MC-GPU_v1.5b.x -m64 -O3 -use_fast_math -I. -I/projects01/sysadmin/stuartb/cuda/cuda-10.2/cuda-samples/common/inc/ -I $MPI_INCLUDE -L $MPI_LIB -lmpi -lz --ptxas-options=-v -gencode=arch=compute_50,code=sm_50 -gencode=arch=compute_61,code=sm_61
set +x
